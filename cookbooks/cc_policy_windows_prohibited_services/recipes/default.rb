...
# wrapper_windows_password_requirements.rb
# Bob Filepp
...
def dig_attr(keys, item)
  keys.each do |key|
    if item.key?(key)
      ...
      puts "key does not exist for #{keys}"
...
if policy_map['components']
  policy_map['components'].each do |item|
    ...
    if handler == 'node' && attribute.exists?
      item['enforced_by'].each do |policy_item|
        ...
        if policy_item['policy']
          include_recipe (policy_item['policy']).to_s
    ...
    next unless handler == 'value' && attribute == value
...
puts 'wrapper windows_password_requirements completed'
